...
export function calcStorageFee(
    ...
    return shr16ceil(
        (stats.bits * prices.bit_price_ps +
            stats.cells * prices.cell_price_ps) *
            duration,
    );
}
...
export function computeGasFee(prices: GasPrices, gas: bigint): bigint {
    ...
    return (
        ...
        (prices.gas_price * (gas - prices.flat_gas_limit)) / 65536n
    );
}
...
export function computeDefaultForwardFee(msgPrices: MsgPrices) {
    return (
        msgPrices.lumpPrice -
        ((msgPrices.lumpPrice * msgPrices.firstFrac) >> BigInt(16))
    );
}
...
export function computeMsgStateInitFwdSizeStats(
    ...
    if (init) {
        ...
        if (init.libraries) {
            ...
            storageStats = storageStats.add(
                collectCellStats(
                    beginCell().storeDictDirect(init.libraries).endCell(),
                    visited,
                    true,
                ),
            );
        ...
        if (init.code) {
            refCount++;
            storageStats = storageStats.add(
                collectCellStats(init.code, visited),
            );
        }
        if (init.data) {
            refCount++;
            storageStats = storageStats.add(
                collectCellStats(init.data, visited),
            );
        }
...
export function computeMessageForwardFees(msgPrices: MsgPrices, msg: Message) {
    ...
    let { stats: storageStats, visited } = computeMsgStateInitFwdSizeStats(
        msg.init,
    );

    const bodyStats = collectCellStats(msg.body, visited, false);
    storageStats = storageStats.add(bodyStats);

    // NOTE: Extra currencies are ignored for now
    let fees = computeFwdFeesVerbose(
        msgPrices,
...
export function computeFwdFees(
    msgPrices: MsgPrices,
    cells: bigint,
    bits: bigint,
) {
...
export function extractComputeGas(tx: BlockchainTransaction): bigint {
...
export function roundFee(fee: bigint) {
    const precision = 1_000_000n;

    return fee + ((precision - (fee % precision)) % precision);
}
