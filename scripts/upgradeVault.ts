...
import { JettonVaultCode } from '../compilables';
...
export async function run(provider: NetworkProvider) {
    ...
    const vaultDeploys = matchDeployFiles('vault').filter(([str]) =>
        str.startsWith('vault'),
    );
    const [, vaultDeploy] = await ui.choose(
        'Choose Vault contract to upgrade',
        vaultDeploys,
        ([filename]) => `${filename}`,
    );
    ...
    ui.setActionPrompt(`Upgrading Vault ${vault.address}...`);
    ...
    await factory.sendAdminAction(
        ...
        vault.address,
        0n,
        buildUpgradeMessage(await JettonVaultCode),
    );
